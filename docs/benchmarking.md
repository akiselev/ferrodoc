...
For FP2, `qualify-baseline-cli` takes the same corpus/executable/engine/model arguments as
`qualify-cli` but passes `--document-profile baseline`, so born-digital pages contribute to OCR
cost and failure measurements. It requires a qualified OCR model or Tesseract installation; the
purpose-built regression engine used by unit tests is never accepted as quality evidence.
