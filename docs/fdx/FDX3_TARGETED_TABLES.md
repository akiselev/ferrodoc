...
The reference `table.rulebased` engine recognizes a deliberately small grammar: two or more
consistent rows and columns, nonempty cells, and `|` separators. It emits `EvidenceContent::Table`
on a new region-owned layer. Every cell records an exact UTF-8 byte span into the pre-existing text
hypothesis. Table and cell geometry inherit the source geometry and quality; the engine never
manufactures cell boxes. Limits are 4 MiB of target text, 4,096 rows, 256 columns, and 65,536 cells.
...
phase. Consequently table-cell association accuracy, ordering/package/pin/electrical-table recall,
