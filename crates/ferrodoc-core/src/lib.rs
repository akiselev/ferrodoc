...
use std::{collections::BTreeMap, fmt, fs::File, io::Read, path::{Path, PathBuf}, str::FromStr};
...
impl Rect {
    ...
    pub fn area(self) -> f32 { self.width.max(0.0) * self.height.max(0.0) }
    ...
    pub fn expand(self, margin: f32, page: Option<Self>) -> Self {
        let mut out = Self::new(self.x - margin, self.y - margin, self.width + 2.0 * margin, self.height + 2.0 * margin);
...
impl Device {
    pub fn is_gpu(&self) -> bool {
        matches!(self, Self::Cuda { .. } | Self::Vulkan { .. } | Self::Metal | Self::Wgpu | Self::Hybrid)
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize, JsonSchema, Default)]
...
impl Bytes {
    ...
    pub const fn mib(value: u64) -> Self { Self(value * Self::MIB) }
    pub const fn gib(value: u64) -> Self { Self(value * Self::GIB) }
    pub fn saturating_sub(self, rhs: Self) -> Self { Self(self.0.saturating_sub(rhs.0)) }
}

impl fmt::Display for Bytes {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.0 >= Self::GIB { write!(f, "{:.2} GiB", self.0 as f64 / Self::GIB as f64) }
        else if self.0 >= Self::MIB { write!(f, "{:.1} MiB", self.0 as f64 / Self::MIB as f64) }
        else if self.0 >= Self::KIB { write!(f, "{:.1} KiB", self.0 as f64 / Self::KIB as f64) }
...
impl FromStr for Bytes {
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        ...
        let n: f64 = number.parse().map_err(|_| CoreError::InvalidQuantity(input.into()))?;
...
impl Digest {
    pub fn sha256_bytes(bytes: &[u8]) -> Self {
        ...
        hasher.update(bytes);
        Self { algorithm: "sha256".into(), hex: hex::encode(hasher.finalize()) }
    }
    ...
    pub fn sha256_file(path: impl AsRef<Path>) -> Result<Self, CoreError> {
        ...
        Ok(Self { algorithm: "sha256".into(), hex: hex::encode(hasher.finalize()) })
    }

    pub fn as_key(&self) -> String { format!("{}-{}", self.algorithm, self.hex) }
}
...
impl BlobRef {
    pub fn file(path: impl Into<PathBuf>, media_type: impl Into<String>) -> Self {
        Self { path: path.into(), offset: 0, len: None, media_type: media_type.into(), digest: None }
    }
}
...
impl Default for ResourceEstimate {
    fn default() -> Self {
        Self {
            peak_ram: Bytes::default(), peak_vram: Bytes::default(), warm_ram: Bytes::default(), warm_vram: Bytes::default(),
            latency_ms: 0, visual_tokens: 0, output_tokens: 0, remote_cost_microusd: 0, quality_hint: 0.5, notes: vec![],
        }
    }
}
...
impl HardwareInventory {
    pub fn conservative_local() -> Self {
        Self {
            logical_cpus: std::thread::available_parallelism().map(|x| x.get()).unwrap_or(1),
...
impl Provenance {
    pub fn native(source: impl Into<String>) -> Self {
        Self { source: source.into(), engine: None, engine_version: None, model: None, model_revision: None, input_digest: None, parameters: BTreeMap::new(), created_at: Utc::now() }
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize, JsonSchema)]
pub struct RunId(pub Uuid);
impl RunId { pub fn new() -> Self { Self(Uuid::new_v4()) } }
